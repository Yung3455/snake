...
//方块构造函数
function Square(x, y, classname) { //构造函数（x，y表示每个小方块的坐标）
    this.x = x * sw;
    this.y = y * sh;
...
function createFood() {
    ...
    var include = true; //循环跳出的条件，true表示食物的坐标在蛇蛇身上（需要继续循环），false表示食物的坐标不在蛇身上（不循环了）
    while (include) {
        x = Math.round(Math.random() * (td - 1)); //Math.round()-->四舍五入
...
Game.prototype.init = function() {
    ...
    document.onkeydown = function(ev) {
        if (ev.which == 37 && snake.direction != snake.directionNum.right) { // 37-->左键,用户按下左键的时候，这条蛇不能是正在往下走
